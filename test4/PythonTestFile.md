...
- **Operation**:
  ...
  - Updates the shortest distances and previous nodes for each node as it explores the graph.
  - Finally, reconstructs and returns the shortest path from `start` to `end`.

### Example Usage
- The code creates a graph with nodes A to F and their connections with edge weights.
