...
7. **validateAndSetFiles(files)**:
   ...
   - Updates the selected files array accordingly.
...
- **on:change={handleFileSelect}**: Calls `handleFileSelect` function when files are selected using the input element.
...
This code snippet defines a file upload component in a Svelte application. Users can drag and drop files or click to select files for upload. The component enforces restrictions on file types and the maximum number of files that can be uploaded.
